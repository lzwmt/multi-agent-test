...
class FlightFetcher:
    ...
    def get_ctrip_price(self, dep_code, arr_code, date_str):
        ...
        try:
            with sync_playwright() as p:
                ...
                # 直接访问搜索结果页
                url = f"https://flights.ctrip.com/online/list/round-{dep_code}-{arr_code}?depdate={date_str}&cabin=Y_S_C_F"
    ...
    def get_complete_data(self, dep_city, arr_city, dep_code, arr_code, date_str):
        """获取完整数据"""
        print("=" * 70)
        print(f"完整数据抓取: {dep_city}({dep_code}) -> {arr_city}({arr_code}), {date_str}")
        print("=" * 70)
    ...
    def monitor_multiple_dates(self, dep_city, arr_city, dep_code, arr_code, days=7):
        """监控多天价格"""
        print("=" * 70)
        print(f"监控 {days} 天价格: {dep_city} -> {arr_city}")
        print("=" * 70)
        
        base_date = datetime.strptime("2026-03-12", "%Y-%m-%d")
        results = []
        ...
        for r in results:
            ...
            highest = r.get('highest_price', 'N/A')
            print(f"{date}: ¥{lowest} - ¥{highest}")
...
if __name__ == "__main__":
    ...
    # 测试多天监控
    results = fetcher.monitor_multiple_dates(
        dep_city="广州",
        arr_city="青岛",
        dep_code="CAN",
        arr_code="TAO",
