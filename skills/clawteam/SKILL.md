...
**Tool Call Safety**: When spawning workers that may use browser/web tools, add safety instructions to prevent infinite loops:
...
**For Subagent Mode (sessions_spawn)**: When using `sessions_spawn` with `runtime="subagent"`, explicitly include tool constraints in the task:

```python
# Safe: Limit tool usage to prevent infinite loops
sessions_spawn(
    runtime="subagent",
    task="""Analyze stocks. 
    
Tool constraints:
- Browser/web tools: Maximum 3 calls, stop if page returns empty
- If data fetch fails, continue with theoretical analysis
- Do not retry the same URL more than once"""
)

# Or completely disable tools:
sessions_spawn(
    runtime="subagent", 
    task="Analyze stocks based on public knowledge. Do NOT use browser, web_search, or any data fetching tools."
)
```
